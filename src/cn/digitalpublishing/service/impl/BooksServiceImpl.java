		...
		try {
            ...
            for (int i = sheet.getFirstRowNum()+1; i <= sheet.getPhysicalNumberOfRows(); i++) {
                ...
                if (row != null) {
                    ...
                    XSSFCell series = row.getCell(2);
                    XSSFCell author = row.getCell(3);
                    XSSFCell publishDate = row.getCell(4);
                    XSSFCell isbn = row.getCell(5);
                    XSSFCell price = row.getCell(6);
                    XSSFCell onPrice = row.getCell(7);
                    XSSFCell offPrice = row.getCell(8);
                    XSSFCell publisher = row.getCell(9);
                    XSSFCell language = row.getCell(10);
                    XSSFCell edition = row.getCell(11);
                    XSSFCell page = row.getCell(12);
                    XSSFCell frame = row.getCell(13);
                    XSSFCell format = row.getCell(14);
                    XSSFCell Sheet = row.getCell(15);
                    XSSFCell size = row.getCell(16);
                    XSSFCell wordNumber = row.getCell(17);
                    XSSFCell cover = row.getCell(18);
                    XSSFCell url = row.getCell(19);
                    XSSFCell category = row.getCell(20);
                    XSSFCell location = row.getCell(21);
                    XSSFCell introduction = row.getCell(22);
                    XSSFCell remark = row.getCell(23);
                    ...
                    if(series != null&&!"".equals(series.toString())){
                        book.setSeries(series.toString());
                    }
                    if(author != null&&!"".equals(author.toString())){
                    book.setAuthor(author.toString());
                    }
                    if(publishDate != null){
                    	if(publishDate.toString().split("/").length==3){
                    		book.setPublishDate(DateFormatUitl.stringToDateFormat(publishDate.toString()));
                    	}else{
                    		String pubdate = publishDate.toString()+"/1";
                    		book.setPublishDate(DateFormatUitl.stringToDateFormat(pubdate));
                    	}
                    }
                    ...
                    if(isbn != null&&!"".equals(isbn.toString())){
                    	...
                    	condition.put("aisbn", isbn.toString());
                    	List<Books> BooksList = this.daoFacade.getBooksDao().getList(condition, "");
                    	if(0<BooksList.size()){
                    		msgother = "Pages.books.isbn.buchongfu";
                     		throw new CcsException(msgother);
                    	}else{
                    ...
                    if(language != null&&!"".equals(language.toString())){
                        book.setLanguage(language.toString());
                    }
                    ...
                    if(size != null&&!"".equals(size.toString())){
                        book.setSize(size.toString());
                    }
                    if(wordNumber != null&&!"".equals(wordNumber.toString())){
                        book.setWordNumber(wordNumber.toString());
                    }
                    ...
                    book.setRemark(remark.toString());
                    }
