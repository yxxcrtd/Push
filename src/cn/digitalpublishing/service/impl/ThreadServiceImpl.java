...
//				this.serviceFactory.getAshgateService().ImportExcel(obj,webRoot, Param.getParam("TPIService.info.directory.config").get("templateCode"));
				...
				//拆分成多条XML信息
					if(template.getType()==0){//图书
						...
						//保存路径
						String storePath =ProcessQueue.WEBROOT+File.separator+ProcessQueue.UPLOADROOT+File.separator+ProcessQueue.SOURCEDATAPATH+File.separator+tSource.getId()+File.separator;
	...
	public void downLoadFile(List<TProduct> list) throws CcsException {
		...
		try{
			...
			//文件保存路径
			String filePath = new StringBuffer().append(ProcessQueue.WEBROOT).append(File.separator).append(ProcessQueue.UPLOADROOT).append(File.separator).append(ProcessQueue.PUSHHOME).append(File.separator).append(source.getId()).toString();
			//执行下载
