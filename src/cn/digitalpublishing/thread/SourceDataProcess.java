					...
					for (FtpDirConfigure ftpDirConfigure : ftpdirconfigurelist) {
						try{
							...
							if(notExistFileNames!=null&&notExistFileNames.size()>0){
								...
								//保存到本地路径
								String localPath =ProcessQueue.WEBROOT+File.separator+ProcessQueue.UPLOADROOT+File.separator+ProcessQueue.SOURCEDATAPATH+File.separator+ftpConfigure.getCode()+File.separator+ftpConfigure.getCode()+File.separator+ftpDirConfigure.getFtpdir()+File.separator;
								//下载文件
